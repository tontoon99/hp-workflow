...
- 有料開始条件は `checkout.session.completed` かつ `payment_status=paid` のみ。
- `R1-R4` は全て `Wait` ノード（Webhook再開）で実装。
- PSI は `url` / `strategy` / `category` を明示実装（mobile/desktop両方）。
- 品質ゲートNGは再生成ループへ戻し、上限超過で `quality_failed` 遷移。
- content lock違反時は `Stop Publication` 経路で公開停止。
